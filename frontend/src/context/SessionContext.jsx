...
export const SessionProvider = ({ children }) => {
  ...
  // Expose explicit function to log state transitions exactly when the user clicks them
  // This prevents hydration/reload from triggering duplicate tags in the database.
  const logStateTransition = useCallback((eventType) => {
    if (!workSessionState.running || !workSessionState.sessionId || !employee?._id) return;
    logWellnessEventApi({
      empId: employee._id,
  ...
  }, [workSessionState.running, workSessionState.sessionId, employee?._id]);
